/**
 * Minimal RFC-6902 JSON-Patch implementation for `glm_apply_patch`.
 *
 * Copied from `integrations/cli/src/lib/json-patch.ts` so the two packages
 * stay independently-versionable. Supports the ops we expect from Claude:
 * `add`, `remove`, `replace`, `move`. `copy` and `test` throw — they're
 * rare in refine flows.
 */
